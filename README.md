...
By default the zero remains with a backslash `\` this was done to prevent a confusion with the danish `Ø` however, when paring the typeface with a monospace that only supports a forwardslash `/` this may create a cognitive dissonance problem. In that occasion you might want to flip the zero by adding `font-variant-numeric: slashed-zero;` to your CSS file.
